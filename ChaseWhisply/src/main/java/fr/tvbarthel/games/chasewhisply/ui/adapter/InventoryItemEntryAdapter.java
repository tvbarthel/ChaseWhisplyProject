...
public class InventoryItemEntryAdapter extends ArrayAdapter<InventoryItemEntry> {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		((TextView) rowView.findViewById(R.id.row_inventory_item_entry_title)).setText(context.getResources().getQuantityText(titleResourceId, 1));
		...
		String stringDroppedBy = context.getString(R.string.inventory_item_can_t_be_dropped);
		...
		if (lootsAndPercents.size() != 0) {
			...
			for (Map.Entry<Integer, Integer> entry : lootsAndPercents.entrySet()) {
				stringDroppedBy += context.getString(R.string.dropped_by_entry, context.getString(entry.getKey()), entry.getValue());
				stringDroppedBy += " ";
			}
